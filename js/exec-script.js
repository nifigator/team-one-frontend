...
$(document).ready(function () {
    ...
    function getOrders(userId) {
        return $.ajax({
            type: 'GET',
            url: '/api/v1.0/management/' + userId + '/issues',
            headers: {
    ...
    function drawOrders(data) {
        ...
        if (data.length) {
            ...
            data.forEach(function (order, idx) {
                ...
                // }, ));
                orderListItemBlock.append($('<div class="dropdown js-change-order-status"><button class="collapse-list__collapse-btn btn-block dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></button><div class="dropdown-menu" aria-labelledby="dropdownMenuButton"><a class="dropdown-item" data-status-id="2" href="#">В работе</a><a class="dropdown-item" data-status-id="3" href="#">Отклонена</a><a class="dropdown-item" data-status-id="4" href="#">Выполнена</a></div></div>'));
                orderListItem.append(orderListItemBlock);
