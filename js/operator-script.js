...
$(document).ready(function () {
    ...
    function drawOrders(data) {
        ...
        if (data.length) {
            ...
            data.forEach(function (order, idx) {
                ...
                // }, ));
                orderListItemBlock.append($('<div class="dropdown js-change-order-status"><button class="collapse-list__collapse-btn btn-block dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></button><div class="dropdown-menu" aria-labelledby="dropdownMenuButton"><a class="dropdown-item" data-for-request="' + order.id + '" data-status-id="2" href="#">В работе</a><a class="dropdown-item" data-for-request="' + order.id + '" data-status-id="3" href="#">Отклонена</a><a class="dropdown-item" data-for-request="' + order.id + '" data-status-id="4" href="#">Выполнена</a></div></div>'));
    ...
    $('#requests').on('click', '.js-change-order-status .dropdown-item', function(evt){
        ...
        var issue = {
            "status_id": parseInt(this.dataset.statusId),
            "issue_id": parseInt(this.dataset.forRequest),
        };
    
        $.ajax({
            type: "PUT",
            url: "https://team-one.kras.ru/api/v1.0/customers/" + customer_id + "/issues/" + issue.issue_id,
